...
## Knowing what will be drawn

Every tool states its construction details up front, so you never have to run it to find out what it makes.

- **Both wall dialogs carry a live "What will be drawn" pane** that updates as you change options — spacing (in inches *and* millimeters), plate/track counts, horizontal blocking and its interval, opening framing, and the corner and T assemblies.
- **Every tool prints the same breakdown to the output** when it runs, in three parts: what was drawn, what you switched off, and what the tool does not generate at all.
- **The limits are stated, not implied.** The most important one: **wall diagonal bracing (contraventamento em X / fita de travamento) is not generated by any tool, in either material** — it has to be added separately. Floors likewise get no openings framed around penetrations, and no roof-plane wind girder is generated.
- Truss roofs do not draw collar ties, ceiling joists, kickers or king posts — those belong to the stick-framing path, and truss mode says so rather than leaving the options looking effective.

`lib/wf_detail_spec.py` is the single source of truth for all of this, so the dialogs and the output can never disagree. Its tests assert that every config field it names actually exists, which is what stops an option from quietly doing nothing.
